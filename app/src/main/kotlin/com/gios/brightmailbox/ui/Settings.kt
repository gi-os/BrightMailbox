...
fun SettingsScreen(vm: MailboxViewModel) {
    ...
    Frame {
        ...
        Column(Modifier.weight(1f).verticalScroll(scroll)) {
            ...
            /*
             * A fictional mailbox, for showing the app to somebody.
             *
             * Confirms in place rather than switching on the first tap, the same as DELETE
             * and JUNK do in the reader: this restarts the app and hides everything on the
             * screen behind it, which is not something to discover by mis-tapping a
             * settings row.
             *
             * The restart is the honest part of the design. The database file is chosen
             * when the app starts and every list is a Flow bound to it, so this cannot be
             * swapped under a running screen — and a demo that half-applied would be worse
             * than one that takes a second to arrive.
             */
            val context = androidx.compose.ui.platform.LocalContext.current
            var confirming by remember { mutableStateOf(false) }
            val on = vm.repo.demoOn

            T(
                if (on) {
                    "You are looking at seven invented letters and twelve invented " +
                        "notices. Your own mail is untouched in a database this build is " +
                        "not opening, and nothing here can reach a server."
                } else {
                    "Swaps your mail for a mailbox that does not exist — enough of it to " +
                        "show somebody what the app does, or to take a screenshot with. " +
                        "Your own mail stays where it is and is never opened while this " +
                        "is on."
                },
                t.detail,
                Secondary,
            ...
            Row(
                Modifier.fillMaxWidth().lightClickable {
                    if (!confirming) {
                        ...
                        vm.repo.setDemo(!on)
                        // Relaunch rather than recreate: the repository is a process-wide
                        // singleton holding the database, so only a new process picks up
                        // the other one.
                        runCatching {
                ...
                T(
                    when {
                        confirming && on -> "RESTART WITH YOUR MAIL?"
                        confirming -> "RESTART INTO THE DEMO?"
                        on -> "LEAVE THE DEMO"
                        else -> "SHOW THE DEMO"
                    },
                    t.button,
                    if (on) Content else Secondary,
                    maxLines = 1,
                )
                if (!confirming) T(if (on) "on" else "off", t.copy, Secondary)
            }
            if (confirming) {
                Spacer(Modifier.height(g * 0.3f))
                T(
                    if (on) "The demo mailbox is deleted. Your accounts come back."
                    else "The app closes and opens again.",
                    t.superfine,
                    Secondary,
                )
            }
